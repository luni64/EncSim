...
class SerialCommand {
  public:
    ...
    void addCommand(const char *command, std::function<void()>);  // Add a command to the processing dictionary.
    void setDefaultHandler(std::function<void(const char *)>);   // A handler to call when no valid command received.
  ...
  private:
    ...
    SerialCommandCallback commandList[20];
    byte commandCount;
